...
int main(int argc, char *argv[])
{
    ...
    char *src;
    char *dst;
    int64_t *pDtsStartTime = NULL;
    int64_t *pPtsStartTime = NULL;
    double starttime;
    double endtime;
    ...
    ret = av_seek_frame(pFormatContext, -1, starttime * AV_TIME_BASE, AVSEEK_FLAG_BACKWARD);
    if (ret < 0)
    {
        av_log(pFormatContext, AV_LOG_ERROR, "Error occurred when seek to starttime%s\n", av_err2str(ret));
        goto _EEROR;
    }
    pDtsStartTime = av_calloc(pFormatContext->nb_streams,sizeof(int64_t));
    for (int t = 0; t < pFormatContext->nb_streams; t++)
    {
        pDtsStartTime[t] = -1;
    }
    pPtsStartTime = av_calloc(pFormatContext->nb_streams,sizeof(int64_t));
    for (int t = 0; t < pFormatContext->nb_streams; t++)
    {
        pPtsStartTime[t] = -1;
    }
    ...
    while (av_read_frame(pFormatContext, &packet) >= 0)
    {
        ...
        if(pDtsStartTime[packet.stream_index] == -1 && packet.dts > 0 )
        {
            pDtsStartTime[packet.stream_index] = packet.dts;
        }
        if (pPtsStartTime[packet.stream_index] == -1 && packet.pts > 0)
        {
            pPtsStartTime[packet.stream_index] = packet.pts;
        }

        pInputStream = pFormatContext->streams[packet.stream_index];
        if(av_q2d(pInputStream->time_base)*packet.pts < endtime)
        {
            av_log(pFormatContext, AV_LOG_DEBUG, "packet pts %lld dts %lld,success\n", packet.pts, packet.dts);
            break;
        }
        ...
        packet.pts = packet.pts - pPtsStartTime[packet.stream_index];
        packet.dts = packet.dts - pDtsStartTime[packet.stream_index];
...
_EEROR:
    ...
    if (pDtsStartTime != NULL)
    {
        av_free(pDtsStartTime);
        pDtsStartTime = NULL;
    }
    if (pPtsStartTime != NULL)
    {
        av_free(pPtsStartTime);
        pPtsStartTime = NULL;
    }
